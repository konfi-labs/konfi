...
export default function Dropzone({
  ...
  const [fileRejections, setFileRejections] = useState<
    FileUploadFileRejection[]
  >([]);
  const _maxSize = maxSize ?? 5242880;
  const effectiveMaxFiles = multiple ? maxFiles : 1;

  const bg = { base: "whiteAlpha.500", _dark: "whiteAlpha.300" };
  const activeBg = { base: "blackAlpha.50", _dark: "blackAlpha.50" };
  ...
  const getFileErrorMessage = (error: FileUploadFileError): string => {
    switch (error) {
      case "FILE_INVALID_TYPE":
        return t("dropzone.fileInvalidType", {
          defaultValue: "Invalid file type",
        });
      case "FILE_TOO_LARGE":
        return t("dropzone.fileTooLarge", {
          defaultValue: "File is too large",
        });
      case "FILE_TOO_SMALL":
        return t("dropzone.fileTooSmall", {
          defaultValue: "File is too small",
        });
      case "TOO_MANY_FILES":
        return t("dropzone.tooManyFiles", {
          defaultValue: "Too many files",
        });
      case "FILE_EXISTS":
        return t("dropzone.fileExists", {
          defaultValue: "File already selected",
        });
      case "FILE_INVALID":
        return t("dropzone.fileInvalid", {
    ...
    <Box>
      ...
      >
        <FileUpload.HiddenInput
          title={t("dropzone.upload", { defaultValue: "Upload" })}
        />
        <FileUpload.Context>
          {({ dragging }) => {
            const dropText = dragging
              ...
              : t("dropzone.dragOrClickToUpload", {
                  defaultValue: "Drag or click to upload files.",
                });
            const borderColor = dragging
              ? { base: "blackAlpha.600", _dark: "blackAlpha.300" }
              : { base: "blackAlpha.900", _dark: "blackAlpha.700" };
      ...
      <Text fontWeight={"600"} fontSize={"sm"}>
        {t("dropzone.maxFileSize", {
          defaultValue: "Maximum file size:",
        })}{" "}
        {new Intl.NumberFormat(i18n.resolvedLanguage, {
          style: "decimal",
          maximumFractionDigits: 2,
          minimumFractionDigits: 2,
        }).format(_maxSize / (1024 * 1024))}{" "}
        MB
      </Text>
      {acceptedFileItems.length >= 1 && (
        <Box mt={"6"}>
          <Heading mt={"2"} size={"sm"}>
            {t("dropzone.acceptedFiles", { defaultValue: "Accepted files:" })}
          </Heading>
          <List.Root>{acceptedFileItems}</List.Root>
        </Box>
      )}
      {fileRejectionItems.length >= 1 && (
        <Box mt={acceptedFileItems.length >= 1 ? "2" : "6"}>
          <Heading mt={"2"} size={"sm"}>
            {t("dropzone.rejectedFiles", { defaultValue: "Rejected files:" })}
          </Heading>
          <List.Root>{fileRejectionItems}</List.Root>
        </Box>
      )}
    </Box>
  );
