...
import { runNoPaymentDocumentReminderWorkflow } from "@/lib/cron/order-reminders/workflow";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const runs = await runForCronTenants(async ({ tenantId }) => {
      const run = tenantId
        ? await start(runNoPaymentDocumentReminderWorkflow, [tenantId])
        : await start(runNoPaymentDocumentReminderWorkflow);
    ...
    console.error(
      "Failed to start no-payment-document reminder workflow",
      error,
    );
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unknown no-payment-document reminder start error.",
      },
      { status: 500 },
    );
  }
