...
const mocks = vi.hoisted(() => ({
  ...
  mockRunMonthlyWhatsNewWorkflow: vi.fn(),
  mockStart: vi.fn(),
}));
...
vi.mock("@/lib/whats-new/monthly-workflow", () => ({
  runMonthlyWhatsNewWorkflow: mocks.mockRunMonthlyWhatsNewWorkflow,
}));
...
function createRequest(search: string = "") {
  return new NextRequest(
    `http://localhost/api/cron/whats-new-monthly${search}`,
    {
      method: "GET",
    },
  );
...
describe("/api/cron/whats-new-monthly GET", () => {
  beforeAll(async () => {
  ...
  beforeEach(() => {
    ...
    mocks.mockStart.mockResolvedValue({ runId: "workflow-run-123" });
  });
  ...
  it("starts the monthly workflow and returns an accepted response", async () => {
    ...
    expect(mocks.mockStart).toHaveBeenCalledWith(
      mocks.mockRunMonthlyWhatsNewWorkflow,
      [true],
    );
