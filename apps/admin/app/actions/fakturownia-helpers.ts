import type { InvoicePosition } from "@konfi/fakturownia/client/models";
...
/**
 * Map an OrderItem to a Fakturownia InvoicePosition.
...
export function mapOrderItemToInvoicePosition(
  ...
  const position: InvoicePosition = {
    name: item.description || item.product?.name || "Unknown product",
    quantity,
    tax: resolveInvoiceTaxRate(item, taxSummary),
    totalPriceGross,
    description: item.combination || item.calculatedCombination || undefined,
  };
  if (productId) position.productId = productId;
  return position;
}
