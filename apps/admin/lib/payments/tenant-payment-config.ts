...
export async function hasTenantStripePaymentConfig(
  context?: TenantContext,
): Promise<boolean> {
  ...
  try {
    const integration = await getTenantPaymentIntegration(
      ...
      tenantStripeIntegrationDocumentId(resolvedContext.tenantId),
    );
    const stripe = getStripeTenantIntegrationMetadata(integration?.metadata);
    ...
    console.error("Failed to check Stripe tenant config:", error);
    return false;
  }
}

export async function hasTenantPrzelewy24PaymentConfig(
  context?: TenantContext,
): Promise<boolean> {
  const resolvedContext = context ?? (await getTenantContextForRequest());
  ...
  try {
    const integration = await getTenantPaymentIntegration(
      resolvedContext.tenantId,
      tenantPrzelewy24IntegrationDocumentId(resolvedContext.tenantId),
    );
    const przelewy24 = getPrzelewy24TenantIntegrationMetadata(
      integration?.metadata,
    );
    ...
    console.error("Failed to check Przelewy24 tenant config:", error);
    return false;
  }
}
...
export async function getStripePaymentCredentials(
  ...
  assertProcessEnvIntegrationAllowed("Stripe", resolvedContext);
...
export async function getPrzelewy24PaymentCredentials(
  ...
  assertProcessEnvIntegrationAllowed("Przelewy24", resolvedContext);
