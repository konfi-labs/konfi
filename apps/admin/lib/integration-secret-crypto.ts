...
function resolveActiveKeyVersion(): { key: Buffer; keyVersion: string } {
  const keyring = readIntegrationSecretsKeyring();
  const explicitVersion =
    process.env.KONFI_INTEGRATION_SECRETS_ACTIVE_KEY_VERSION?.trim();

  if (explicitVersion) {
    const key = keyring.get(explicitVersion);

    if (!key) {
      throw new Error(
        `KONFI_INTEGRATION_SECRETS_ACTIVE_KEY_VERSION "${explicitVersion}" is not present in KONFI_INTEGRATION_SECRETS_KEYRING.`,
      );
    }

    return { key, keyVersion: explicitVersion };
  }

  if (keyring.size === 1) {
    const [[keyVersion, key]] = keyring.entries();
...
export function encryptIntegrationSecret({
  ...
  const { key, keyVersion } = resolveActiveKeyVersion();
