...
Sentry.init({
  ...
  tracesSampler: (samplingContext) => {
    ...
    // Sample all critical business operations (checkout, orders, cart, etc.)
    if (name.includes("checkout") || name.includes("order") || name.includes("cart") || attributes?.flow === "checkout") {
      return 1.0;
    }
