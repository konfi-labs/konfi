...
import { T_STORE_GENERAL_CONDITIONS_OF_SALE } from "@konfi/utils";
...
export default async function Page({ params }: { params: Params }) {
  return (
    <Suspense fallback={<Skeleton w={"100%"} h={"100vh"} />}>
      <GeneralConditionsOfSalePageContent params={params} />
    </Suspense>
  );
}

async function GeneralConditionsOfSalePageContent({
  params,
}: {
  params: Params;
}) {
  const { lng } = await params;
  const source = await fetchPageContent(
...
export async function generateMetadata({
  ...
  const { lng } = await params;
  return await fetchMetadata(T_STORE_GENERAL_CONDITIONS_OF_SALE, lng);
}
