...
import { T_STORE_REGULATIONS } from "@konfi/utils";
...
export default async function Page({ params }: { params: Params }) {
  return (
    <Suspense fallback={<Skeleton w={"100%"} h={"100vh"} />}>
      <RegulationsPageContent params={params} />
    </Suspense>
  );
}

async function RegulationsPageContent({ params }: { params: Params }) {
  const { lng } = await params;
  const source = await fetchPageContent(T_STORE_REGULATIONS, lng);
...
export async function generateMetadata({
  ...
  const { lng } = await params;
  return await fetchMetadata(T_STORE_REGULATIONS, lng);
}
