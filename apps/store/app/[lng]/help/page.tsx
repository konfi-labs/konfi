...
import { T_STORE_HELP } from "@konfi/utils";
...
export default async function Page({ params }: { params: Params }) {
  return (
    <Suspense fallback={<Skeleton w={"100%"} h={"100vh"} />}>
      <HelpPageContent params={params} />
    </Suspense>
  );
}

async function HelpPageContent({ params }: { params: Params }) {
  const { lng } = await params;
  const source = await fetchPageContent(T_STORE_HELP, lng);
...
export async function generateMetadata({
  ...
  const { lng } = await params;
  return await fetchMetadata(T_STORE_HELP, lng);
}
